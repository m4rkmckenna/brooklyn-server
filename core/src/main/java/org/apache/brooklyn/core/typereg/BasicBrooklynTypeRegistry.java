...
public class BasicBrooklynTypeRegistry implements BrooklynTypeRegistry {
    ...
    static final Counter typeRegistryAdd = Counter.build().name("addToLocalUnpersistedTypeRegistry_count").labelNames("symbolic_name")
            .help("addToLocalUnpersistedTypeRegistry count per symbolic name")
            .subsystem(BasicBrooklynTypeRegistry.class.getName().replace("org.apache.brooklyn", "o_a_b").replace('.', '_'))
            .register();
    ...
    public <T> T create(RegisteredType type, RegisteredTypeLoadingContext constraint, Class<T> optionalResultSuperType) {
        ...
        return new RegisteredTypeKindVisitor<T>() {
            ...
            protected T visitSpec() {
                return (T) createSpec(type, constraint, (Class) optionalResultSuperType);
            }
    ...
    public void addToLocalUnpersistedTypeRegistry(RegisteredType type, boolean canForce) {
        typeRegistryAdd.labels(type.getSymbolicName()).inc();
